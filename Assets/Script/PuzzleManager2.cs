...
public class PuzzleManager2 : MonoBehaviour
{
    ...
    public GameObject puzzlePanel;               // Il contenitore del puzzle
    public CanvasGroup puzzleCanvasGroup;        // Per il fade
    ...
    private bool[] unlockedPieces = new bool[5];
    private bool[] hasAnimatedPiece = new bool[5];  // 🔄 Tiene traccia dei pezzi già animati
    ...
    void Start()
    {
        ...
        puzzlePanel.SetActive(false);            // Nasconde il puzzle all'inizio
        puzzleCanvasGroup.alpha = 0f;            // Trasparente
        puzzlePanel.transform.localScale = Vector3.one * 0.8f; // Zoom iniziale
    ...
    private IEnumerator ShowPuzzleWithAnimation(float delay)
    {
        ...
        if (lastUnlockedIndex >= 0 && lastUnlockedIndex < puzzlePieces.Length && !hasAnimatedPiece[lastUnlockedIndex])
        {
            ...
            piece.transform.localScale = Vector3.one;
            hasAnimatedPiece[lastUnlockedIndex] = true; 
        }
